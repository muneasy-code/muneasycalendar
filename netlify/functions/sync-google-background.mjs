...
export default async (request) => {
  ...
  try {
    ...
    // Status sofort auf "syncing" setzen
    await fetch(
    ...
    await fetch(
      `${supabaseUrl}/rest/v1/calendars?id=eq.${calendar.id}`,
      {
        ...
        body: JSON.stringify({
          ...
          google_sync_status: "done",
          google_sync_inserted: inserted,
          google_sync_updated: updated,
          google_sync_deleted: deleted,
          google_sync_error: null
        })
      }
    ).catch(() => {});
    ...
    // Wenn möglich Fehlerstatus serverseitig festhalten
    try {
      const tokenForError = new URL(request.url).searchParams.get("token");
      if (tokenForError && supabaseUrl && supabaseKey) {
        const sbHeaders = supabaseHeaders(supabaseKey);
        const r = await fetch(
          `${supabaseUrl}/rest/v1/calendars?token=eq.${encodeURIComponent(tokenForError)}&select=id`,
          { headers: sbHeaders }
        );
        const rows = await r.json();
        if (r.ok && rows?.[0]?.id) {
          await fetch(
            `${supabaseUrl}/rest/v1/calendars?id=eq.${rows[0].id}`,
            {
              method: "PATCH",
              headers: { ...sbHeaders, "Content-Type": "application/json" },
              body: JSON.stringify({
                google_sync_status: "error",
                google_sync_error: String(error?.message || "Google Sync fehlgeschlagen.").slice(0, 1000)
              })
            }
          );
        }
    ...
    return json(
      { success: false, error: error?.message || "Google Sync fehlgeschlagen." },
      500
    );
  }
};
